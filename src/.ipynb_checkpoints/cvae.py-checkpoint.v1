...
class CVAE(object):
    ...
    def encoder(self, X_in, cond, input_dim):
        with tf.compat.v1.variable_scope("encoder", reuse=None):
            ...
            epsilon = tf.random.normal(tf.stack([tf.shape(x)[0], self.n_latent]))
    ...
    def train(self, data, data_cond, n_epochs=10000, learning_rate=0.005,
        ...
        decoded_loss = tf.reduce_sum(tf.math.squared_difference(unreshaped, Y_flat), 1)
